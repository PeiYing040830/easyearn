...
EasyEarn is not developed by a dedicated development team, separate Quality Assurance (QA) team, or enterprise level infrastructure, but by one single student. This means that only a certain degree of parallel development, testing coverage and optimisation can be achieved during the project period. Thus, it is important to prioritize core functions of the system carefully to make sure that a functional and testable system can be built within the limited resources at hand (Pressman & Maxim, 2020).
...
| --- | --- | --- | --- |
| Time<br>Constraint | Established a fixed 26-week academic project deadline | Restricts the development, testing and refinement that can be achieved during the life of the project. | Continue development in phases for features or improvements not within the scope of the project. |
...
Five complementary testing methods were employed that mimic the overall software testing concepts of ISO/IEC/IEEE 29119-1 (2022): System Testing, User Acceptance Testing (UAT), Usability Testing, Security Testing and Compatibility Testing. The overall evaluation of the EasyEarn system is made through each method that evaluates a different quality aspect such as functional correctness, user acceptance, usability, security and cross-environment behaviour. The technical testing activities were done by the project author as it is a solo development project, and UAT was done with external participants.

### 4.3.1 System Testing

Seven categories of system tests were conducted to ensure full end-to-end functionality of EasyEarn with respect to functional requirements: E2E Workflow, Integration, Performance, Recovery, Business Logic, Reporting and Compliance. All 34 test cases returned a Pass result and were designed and executed under this section. Security Testing (29 test cases, SEC-001 to SEC-029) and Compatibility Testing (23 test cases, CT-001 to CT-023) are each covered separately in Sections 4.3.4 and 4.3.5, respectively, as they are different testing methods and have their own test workbooks. The seven categories of System Testing are summarised in Figure 4.2, and the individual test cases with their scenarios, test types and outcomes are provided in Figures 4.3 to 4.36.
...
Takes a test of the jsPDF + html2canvas resume export functionality, checking it generates within the set time limit and is not clipped. The PDF resume export was successful, and the generated document was not clipped, as can be seen in Figure 4.17; it completed export within the target timing.
...
Tests ensuring that only valid forward status changes can be made through the employer UI, with a verifier that reversal of a completed application or a seeker self-editing are not allowed. Figure 4.25 is the employer UI that can only accept valid forward changes in status, which prevents the seekers from changing status and editing their own applications.
...
Tests whether a job seeker submits a suspicious listing, he is saved correctly, and the admin can see him soon after he submits the listing. The listing report of a suspicious job seeker is saved and displayed instantly in the admin's report queue as illustrated in Figure 4.30.
...
Tests where sensitive employer verification documents are separated per account and can only be read by the account owner and admin, as per the Malaysian PDPA. The PDPA is respected by making the employer verification documents limited to the account owner and admin, as shown in Figure 4.34.
...
Tests that will change access due to changes in access status will be reported and will be recorded as rejected status notes and displayed to the affected employer. The audit trail includes a record of the access-status change, and the rejection notes are shown to the employer concerned as shown in Figure 4.35.
...
Admin-focused compliance pages that consistently appear with compliance reminders that align with the Gig Workers Act 2025 (transparent terms, payment clarity, dispute handling) rather than on one single page. The admin compliance pages will include reminders about the Gig Workers Act 2025 regarding terms, clarity of payment, and dispute resolution, as illustrated in Figure 4.36.
...
Completed has been applied and reviewed in both applications, with the "Confirm Payment Received" action in Cashier and a "Rated" tag in Barista. The Applications page includes both completed Applications: Confirm Payment Received action and the Rated tag as shown in Figure 4.41.
...
The count badge for all 3 saved jobs was displayed under the Saved Jobs tab. Each card was confirmed as having a Remove option, and it was clickable. The Saved Jobs tab appears with the count badge and Remove button on the job cards as shown in Figure 4.42.
...
Based on the Interview Summary, the number of confirmed interviews was 2, the number of upcoming interviews was 0, and the number of completed interviews was 2. For both interviews, the correct date and time were specified, along with a working link to Google Meet, and there were actions of View Application and Open Chat. The Interview Summary appears in Figure 4.44 along with the two completed interview records with the date, time, and Google Meet link.
...
The Profile page displayed a 100% Profile Strength with 3 ratings and 2 completed gigs and 2 applications, and 3 saved jobs. All three Edit Info, Skills & Experience, Education Background and Set Availability sections were accurate, as were they in the real saved data. On the Profile page, you can see the Profile Strength indicator and the Edit Info, Skills & Experience and the Availability sections as in Figure 4.48.
...
Successfully logged in with existing Employer Account (W&X Bakery), then successfully redirected to Employer Dashboard, then successful logon with the same Employer Account across the other Employer Pages visited. The Employer Dashboard will appear as shown in Figure 4.50 after logging in, and will show that the current session is W&X Bakery.
...
Manage Jobs has a total of 2 Published, 0 Pending Review, 1 Expired and 0 Closed. A new Create or Edit a Listing form was looked at, but no new job was submitted this round. The Create or Edit a Listing form is displayed with the Manage Jobs status summary as shown in Figure 4.52.
...
The Users summary card value was in line with the Users page's actual value. The Users count in the Users page matches the figure in the Users summary card in the Admin Dashboard as displayed in Figure 4.63.
...
The Account Status was automatically updated to "Locked" upon clicking Lock Account and to "Active" upon clicking Unlock Account. After Lock and Unlock actions, the CarePlus account status has changed from Locked to Active, as illustrated in Figure 4.64.
...
| H6 - Recognition Rather Than Recall | None identified | Persistent navigation and dashboard summaries requires less memory. |
| H7 - Flexibility and Efficiency of Use | None identified | The novice and frequent users are able to use filter, dark mode and saved-jobs without issues. |
...
Other users' notification rows were not returned to the client because the notifications_select_own policy filtered the SELECT so only rows for the current user were returned. When you view the notifications query, only the rows that are for the authenticated user will be returned, as displayed in Figure 4.77.
...
For the applications_update_employer policy on job employer_id, the application-faced status transitions are prevented, but there is no field-level constraint preventing the direct API call by the seeker, so the risk remains as described in Chapter 5.3.1 Figure 4.84 displays the application status field unchanged if it is accessed via the update path that is scoped to the employer.
...
This category will assess the consistency of the basic functionality of EasyEarn that is dependent on CSS, JavaScript and the browser, across the browsers tested: Google Chrome, Mozilla Firefox and Microsoft Edge. Safari did not go into actual testing since it was not available on the testing devices. Its expected compatibility is based on the use of normal Web technologies, and not on direct compatibility checking.
...
In Chrome, Firefox and Edge, the datetime-local input opened and saved correctly and the date selected by the user was passed to the function updateInterviewSchedule() and put into the application's table. Safari also natively includes input of type datetime-local as required by the web standard, which however was not tested on the testing devices employed. To schedule an interview use the date and time picker as illustrated in Figure 4.125.
...
The Resume Builder takes a Job Seeker's profile (profile summary, skills, education, availability and completed work history) and renders it as a snapshot on the browser, scaling it to 2x with html2canvas (von Hertzen, 2025) and embedding it into an A4-formatted document with jsPDF (Hall, 2025). The result is a PDF resume with the populated Skills section (friendly communication, time management, knowledge of POS systems), Education (SPM from SMK Bercham in 2021), and Availability (Mon, Wed, Fri, Sun, Flexible), as shows in Figure 4.129.
